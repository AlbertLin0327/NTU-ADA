...
double job_s_weight = 0.49;
double job_w_weight = 1;
...
struct Slice {
  ...
  int addJob(Job job) {
	...
	++t;
      ...
      fillSlice(idle, t-1, cur.d);
      job.oper[cur.idx].start_time = t - 1;
      ans[job.idx].oper[cur.idx].start_time = t - 1;
      ans[job.idx].oper[cur.idx].used_slice = idle;
      ++job.done;
      if (t - 1 + cur.d > ret) ret = t - 1 + cur.d;
